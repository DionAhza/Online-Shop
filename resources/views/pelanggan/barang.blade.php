...
<div class="container mt-5 d-flex">
    <div class="card mb-4 shadow-sm border-0" style="max-width: 18rem;">
        @if (Auth::check() && (Auth::user()->id == $barang->user_id || Auth::user()->role == 'admin'))
            <div class="d-flex">
                    ...
                    <div class="modal-dialog">
                      <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form action="{{ route('barang.update',$barang->id) }}" method="POST" enctype="multipart/form-data" class="p-5">
                                @csrf
                                @method('PUT')
            ...
            @endif
        @if($barang->gambar)
            <img src="{{ asset('img/' . $barang->gambar) }}" alt="{{ $barang->nama_barang }}" class="img-fluid card-img-top w-110 mx-auto d-block p-3" style="object-fit: cover; height: 200px;">
        @endif
    
        <div class="card-body text-center">
            <h5 class="card-title">{{ $barang->nama_barang }}</h5>
            <a href="{{ route('toko', $barang->user_id) }}" class="hover:underline">
            <p class="text-muted">Penjual: <strong>{{ $barang->user->name }}</strong></p>
        </a>
            @php
                $pembayaran = json_decode($barang->bayar);
            @endphp
            <p class="card-text">Metode Pembayaran : {{ implode(', ', $pembayaran) }}</p>
            <p class="card-text">Harga: <strong>Rp {{ number_format($barang->harga, 0, ',', '.') }}</strong></p>
            <p class="card-text">Stok: <strong>{{ $barang->stock }}</strong></p>
        </div>
    </div>
 ...
 <div class="mt-auto pb-4 ml-3">
    ...
    <form action="{{ route('cart.add') }}" method="POST" class="mt-2">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4 mt-2">
            <a href="{{ route('home') }}" class="btn btn-primary ">&laquo; Back</a><br>
